...
```html
<div class="p-md">
  Padded on all sides by var(--gap-md).
</div>
```

### Inline / block axes

Use `px-{size}` for horizontal padding (inline axis) and `py-{size}` for vertical (block axis). These follow logical properties — they flip automatically in RTL languages.
...
<div class="pt-sm pb-lg">
  Tight on top, generous on bottom.
</div>
```
