...
Popovers and menus render dark-on-light by default (a common design touch) via a container style query (`@container style(--use-inverted-popovers: "true")`). To opt a subtree out, flip the flag or use the `data-use-inverted-popovers` attribute:
...
```html
<div data-use-inverted-popovers="false">…</div>
```
