...
_In a typical setup, changing a corporate color is a search-and-replace across files. With Zazz, you change one variable and the rebrand is done: in Figma, in Webflow, and in code at once._
...
_If you want a louder rebrand, change the corporate scales. If you want denser layouts, change `--spacing-interval`. If you want softer corners, change `--radius-multiplier`. The components don't care._
...
:root {
  ...
  --shadow-md: 0 4px 8px oklch(from var(--black) l c h / 0.08);
}
```

_Change `--neutral-950`, and every shade overlay, every text color, every shadow that derives from it shifts in lockstep. No hunt-and-fix._
...
_Designers switch the Figma mode and the file recolors. Developers toggle a data attribute and the app recolors. Same variables, same outcome, no per-component logic._
...
_The same name lives in every tool. You're not learning four token languages. You're learning one._
...
<Cards>
  <Card
    ...
    description="Base, overlay, brand, status. And the modes they switch between."
  />
  <Card
    title="Corporate"
    href="/docs/foundations/corporate"
    description="Brand color scales for primary, secondary, tertiary, and beyond."
  />
  <Card
    title="Grayscale"
    href="/docs/foundations/grayscale"
    description="Neutrals, and the shade and tint scales derived from them."
  />
  <Card
    title="Typography"
    href="/docs/foundations/typography"
    description="Family, fluid size, weight, tracking, leading, paragraph spacing."
  />
  <Card
    title="Spacing"
    href="/docs/foundations/spacing"
    description="Modifiers, semantic gap-*, and the step-* scale."
  />
  <Card
    title="Radius"
    href="/docs/foundations/radius"
    description="Multiplier, semantic scale, and per-component radii."
  />
  <Card
    title="Layout"
    href="/docs/foundations/layout"
    description="Containers, article widths, and breakpoints for calcs and container queries."
  />
  <Card
    title="Effects"
    href="/docs/foundations/effects"
    description="Shadows and states. Focus rings and feedback."
  />
</Cards>
