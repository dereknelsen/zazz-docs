...
`reveal.js` ships inside the single [`zazz.js`](/docs/getting-started/head#script-load-order) module; its styles are part of `zazz.css`, which `@import`s `ui/_reveal.css`. The normal setup is one link to `zazz.css` plus the module:
