...
Native `<dialog>` (when opened with `showModal()`) and `[popover]` elements render in the browser's top layer, so they sit above everything in the utility scale regardless of `z-index`. See [\_reset.css](app/zazz/styles/_reset.css) for their open/close wiring.
