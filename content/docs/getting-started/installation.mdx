...
You _can_ instead link each `_*.css` file individually (with `preload` hints) to avoid `main.css`'s `@import` chain, but with HTTP/2–3 multiplexing and server compression it's rarely worth the upkeep. If you do, keep the cascade order: `_layers.css` first, then `_properties.css`, `_variables.css`, `_reset.css`; component files in any order; `_utilities.css` last.
