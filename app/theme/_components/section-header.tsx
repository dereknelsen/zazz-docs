...
export function TableHeader({ columns }: TableHeaderProps) {
  return (
    <div className="sticky top-0 z-10 grid items-center gap-4 border-b border-fd-border/40 bg-fd-card/60 px-6 py-2 text-[11px] font-medium uppercase tracking-[0.08em] text-fd-muted-foreground backdrop-blur" style={{ gridTemplateColumns: columns.map((c) => c.className ?? "1fr").join(" ") }}>
      {columns.map((c) => (
