...
Every gap, padding, and margin in Zazz resolves through three layers: a single global interval, a numbered scale derived from it, and a small set of named gaps that map onto specific scale steps. The point is that one number — `--spacing-interval` — controls every space in the product. Change it, everything moves.
...
<Accordions>
  <Accordion title="Can I add a new semantic gap like `--gap-2xl`?">
    ...
    If you find yourself adding the same custom gap to every project, consider whether the default scale needs an additional rung. But most teams should resist over-systematizing the semantic layer — five well-chosen gaps cover more cases than fifteen ad-hoc ones.
  </Accordion>
</Accordions>
...
If you're picking a step for a designer-facing spacing decision, ask whether the value should track the named gaps. If yes, use the gap. If no — if it's something deliberately off-grid for a single component — use the step.
...
<Cards>
  <Card title="Layout" href="/docs/foundations/layout" description="Containers and breakpoints that compose with spacing tokens via `min()` calcs." />
  <Card title="Radius" href="/docs/foundations/radius" description="Radii derive from the same step scale, so corners and gaps stay proportional." />
  <Card title="Customization" href="/docs/practices/customization" description="Retuning the interval per project, app, or scoped section." />
</Cards>
