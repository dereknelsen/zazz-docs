...
- Arrow keys are wired by the Zazz carousel script: `ArrowLeft` / `ArrowRight` scroll when focus is inside the carousel, or when it lives in an open `<dialog>` (e.g. lightbox). Opt out with `data-embla-keyboard="false"`.
- Embla respects `prefers-reduced-motion` for its scroll animations.
