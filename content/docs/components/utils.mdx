...
`utils.js` ships inside the single [`main.js`](/docs/getting-started/head#script-load-order) module and loads automatically — `embla.js` `import`s it directly, so there's no load order to manage by hand:
