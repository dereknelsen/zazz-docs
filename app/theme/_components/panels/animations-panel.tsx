...
export function AnimationsPanel() {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-auto">
      <SectionHeader title="Animations" description="Spring easing curve and default transition timing/duration." />
      <div className="flex flex-col gap-5 px-6 py-4">
        <section className="rounded-xl border border-fd-border/30 bg-fd-card/30 p-4">
          ...
          <label className="mb-3 flex flex-col gap-1.5">
            <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-fd-muted-foreground">--spring-easing</span>
            <textarea
              value={animations.springEasing}
              onChange={(e) => update((p) => ({ ...p, animations: { ...p.animations, springEasing: e.target.value } }))}
              rows={12}
              spellCheck={false}
          ...
          <div className="grid grid-cols-2 gap-3">
            <TextInput
              ...
              onChange={(v) => update((p) => ({ ...p, animations: { ...p.animations, springDuration: v } }))}
            />
            <TextInput
              mono
              label="--default-transition-duration"
              value={animations.defaultDuration}
              onChange={(v) => update((p) => ({ ...p, animations: { ...p.animations, defaultDuration: v } }))}
            />
          </div>
        </section>
        ...
        <section className="rounded-xl border border-fd-border/30 bg-fd-card/30 p-4">
          ...
          <TextInput
            ...
            onChange={(v) => update((p) => ({ ...p, animations: { ...p.animations, defaultTimingFunction: v } }))}
          />
        </section>
      </div>
    </div>
