...
The Zazz reset sets `header` and `section` elements to `position: relative` so they establish stacking contexts predictably without an explicit class. `main` is positioned `relative` for the same reason. See [\_reset.css](app/zazz/styles/_reset.css).
