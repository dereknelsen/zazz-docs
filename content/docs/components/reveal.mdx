...
`reveal.js` ships inside the single [`main.js`](/docs/getting-started/head#script-load-order) module; its styles are part of `main.css`, which `@import`s `ui/_reveal.css`. The normal setup is one link to `main.css` plus the module:
