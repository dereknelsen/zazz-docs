...
Scripts in `zazz/scripts/` are served directly at `/zazz/scripts/*.js`, with no bundler or transpiler. They are native ES modules and run as-is in modern browsers. A single entry module, `main.js`, `import`s every component script, so a page loads behavior with one `<script type="module" src="./zazz/scripts/main.js">` tag (see [Structuring your head tag](/docs/getting-started/head#script-load-order)).
...
Cross-script dependencies are real ES `import`s, so the module graph resolves order — you load `main.js` and nothing else needs sequencing:
...
The one ordering the module graph can't enforce is the **Embla CDN UMD bundles**: `embla.js` reads them as globals, so their `<script defer>` tags must precede the `main.js` module in the document. Where these tags go is covered in [Structuring your head tag](/docs/getting-started/head#script-load-order).
