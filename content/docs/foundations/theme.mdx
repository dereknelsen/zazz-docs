...
:root {
  ...
  --background: light-dark(var(--white), var(--neutral-950));
  /* …other theme roles use light-dark(light, dark) */
}

.dark {
  color-scheme: dark; /* flips light-dark() tokens for this subtree */
}
```

System dark mode follows `color-scheme` on `:root` (no duplicate `@media` overrides). Force dark by adding `.dark` on an ancestor.

To add a new mode (high-contrast, brand-takeover, holiday), add another `light-dark()` pair or a scoped `color-scheme` override. Components never branch on mode; they consume theme tokens and trust them.
