...
You _can_ instead link each `_*.css` file individually (with `preload` hints) to avoid `main.css`'s `@import` chain, but with HTTP/2-3 multiplexing and server compression it's rarely worth the upkeep. If you do, keep the cascade order: `base/_layers.css` first, then `base/_variables.css`, `base/_reset.css`, `base/_typography.css`, `base/_view-transitions.css`; the `ui/_*.css` component files in any order; then `base/_utilities.css` and `base/_layout.css` last.
