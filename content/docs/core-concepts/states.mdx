...
You rarely write focus styles. The [reset](/docs/foundation/reset) applies a `:focus-visible` ring to every focusable element, so keyboard users get a clear, consistent indicator everywhere. And `:focus-visible` means it shows for keyboard navigation without firing on every mouse click.
