...
export function LayoutPanel() {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-auto">
      <SectionHeader
        title="Layout"
        description="Container widths and breakpoints. Breakpoints are for calcs and container queries, not @media."
      />
        ...
        <section className="rounded-xl border border-fd-border/30 bg-fd-card/30 p-4">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <TextInput
              ...
              onChange={(v) => update((p) => ({ ...p, layout: { ...p.layout, article: v } }))}
            />
            <TextInput
              mono
              label="--container"
              value={layout.container}
              onChange={(v) => update((p) => ({ ...p, layout: { ...p.layout, container: v } }))}
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-5">
            {(Object.keys(layout.screens) as (keyof typeof layout.screens)[]).map((k) => (
              <TextInput
                ...
                mono
                label={`--screen-${k}`}
                value={layout.screens[k]}
                onChange={(v) => setScreen(k, v)}
              />
            ))}
          </div>
        </section>
