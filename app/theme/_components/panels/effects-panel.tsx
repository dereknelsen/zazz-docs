...
export function EffectsPanel() {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-auto">
      <SectionHeader title="Effects" description="Focus rings and layered drop shadows. Multi-layer shadows are comma-separated." />
      <div className="flex flex-col gap-5 px-6 py-4">
        <section className="rounded-xl border border-fd-border/30 bg-fd-card/30 p-4">
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-fd-muted-foreground">--focus-ring</span>
            <textarea
              value={effects.focusRing}
              onChange={(e) => update((p) => ({ ...p, effects: { ...p.effects, focusRing: e.target.value } }))}
              rows={2}
              spellCheck={false}
