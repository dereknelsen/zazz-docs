...
interface PreviewProps {
  /** Example id like `button/variants` — reads zazz/examples/button/variants.html. */
  src: string;
  align?: "start" | "center";
  className?: string;
}
 ...
 * example from the centralized `zazz/examples/` source and renders it two ways
  ...
  if (html == null) {
    return (
      <div className="not-prose rounded-md border border-fd-destructive/40 bg-fd-destructive/5 p-4 text-sm text-fd-destructive">
        Example <code>{src}</code> not found in <code>zazz/examples</code>.
      </div>
    );
  }
