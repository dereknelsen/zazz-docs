...
Zazz ships **eight example components** under `app/zazz/components/` (each a `.tsx`) paired with a stylesheet under `app/zazz/styles/` (each a `_*.css` in `@layer components`):
...
| Form family     | `form.tsx`            | `_form.css` + `_input.css` / `_textarea.css` / `_select.css` / `_input-group.css` / `_radio.css` (checkbox & switch in `_reset.css`) | native form controls, customizable `<select>`, `field-sizing`, `:user-invalid` |
...
1. **A block class + BEM sub-elements.** `.button`, `.dialog` + `.dialog__header` / `.dialog__body` / `.dialog__footer`, `.navigation-menu` + `.navigation-menu__list` / `__item` / `__trigger` / `__popover`. Sub-elements are `block__element`; there are no `block--modifier` classes (see #3).
...
3. **Variants and sizes are data attributes, not modifier classes.** `[data-variant="primary|muted|ghost|link"]`, `[data-size="sm|icon|icon-sm"]`, plus component-specific ones: `[data-side]`/`[data-align]` (dropdown), `[data-align]`/`[data-layout]`/`[data-size]` (navigation menu), `[data-size="large|screen"]` (dialog), `[data-inline]` (a `.field` with the control beside its label) and `[data-align]` (input-group addon placement). In markup: `class="button" data-variant="primary"` — **not** `class="button button-primary"`.
...
5. **Native HTML / platform APIs carry the behavior — no JS, no Radix.** `<details>` for the accordion, native `<dialog>` + Invoker Commands for the dialog, the Popover API + CSS anchor positioning for dropdown and navigation menus, `:has()` for open-state styling. These are Baseline-supported and work in plain HTML and Astro identically.
...
- **Panel layout** (`data-layout` on `.navigation-menu__viewport`): none → single-column link list; `featured` → two columns, a tall `.navigation-menu__featured` callout (gradient via `color-mix`) beside a `.navigation-menu__link` grid.
- **Size** (`data-size="full"` on the popover): a full-width mega-menu anchored to `--navigation-menu-root`.
...
## Form family — `.input` · `.textarea` · `.select` · `.input-group` · checkbox · switch · `.radio`

A set of native form controls styled from Zazz tokens. No JS drives behavior — the browser handles validation, the customizable `<select>` picker, `field-sizing` growth, and checked/indeterminate states. The cross-control glue (shared tokens, the disabled state, the `.field` wrapper, validation) lives in `_form.css`; each control then lives in its own partial.

### Shared `--field-*` tokens

`.input`, `.textarea`, `.select`, and `.input-group` all open with the same locals (declared once in `_form.css`) so a text input, a dropdown, and a message box read as one family — and overriding one local retunes the whole set:

```css
:where(.input, .textarea, .select, .input-group) {
...
`.field` is an optional layout helper — a grid stacking `.field__label` (row 1), the control (row 2), and `.field__hint` / `.field__error` (shared row 3, so only the visible message takes space). `[data-inline]` flips it to a row for checkbox/radio/switch.
...
Validation hangs off **`:user-invalid`** — the error reveals (and the hint hides) only after the user *commits* a value (blur/submit), never while typing. An invalid control flips its border, ring, and label to `--destructive`; the hint↔error swap animates via `@starting-style` + `content-visibility: allow-discrete`.
...
<label class="input-group">
  ...
  <span class="input-group__addon"><svg><!-- leading icon --></svg></span>
  <span class="input-group__addon" data-align="inline-end">
    <button class="button" data-variant="ghost" data-size="icon-sm" type="button" aria-label="Go"><svg>…</svg></button>
  </span>
</label>
```

- **Addons come *after* the control in the DOM** (predictable tab order); `data-align` positions them: `inline-start` (default) · `inline-end` · `block-start` · `block-end`. The `block-*` values span a full-width row — that's how a `.textarea` grows a toolbar above or below it.
...
`appearance: none` redraws the circle; the checked dot is a `radial-gradient` so it tracks `--primary-foreground`, not color alone. Group by shared `name`; `.radio-group` is a flex-column `<fieldset>` for an accessible labelled set.

```html
<fieldset class="radio-group">
  <legend class="text-eyebrow text-muted-foreground mb-xs">Billing</legend>
  <label class="field" data-inline>
