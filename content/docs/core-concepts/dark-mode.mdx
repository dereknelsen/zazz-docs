---
title: Dark mode
description: Dark mode is on from the first line. Use a role color and both themes are correct for free — no .dark overrides, no duplicated styles.
---
...
Out of the box, the page follows the operating system's light/dark setting. Nothing required beyond the [`color-scheme` meta tag](/docs/core-files/head) in your `<head>`, which paints the right canvas, scrollbars, and form controls before your CSS even loads.
...
Popovers and menus render dark-on-light by default — a common design touch — via a container style query (`@container style(--use-inverted-menu: "true")`). To opt a subtree out, flip the flag:

```html
<div style="--use-inverted-menu: 'false'">…</div>
```
...
.dark .panel { background: var(--neutral-900); }
