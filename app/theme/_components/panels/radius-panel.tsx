...
export function RadiusPanel() {
  ...
  const setPrimitive = (k: keyof typeof radius.primitives, v: string) =>
    update((p) => ({
      ...p,
      radius: { ...p.radius, primitives: { ...p.radius.primitives, [k]: v } },
    }));
  ...
  return (
    <div className="flex flex-1 flex-col overflow-auto">
      <SectionHeader
        title="Radius"
        description="Border-radius multiplier, semantic scale, and component primitives."
      />
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
            {(Object.keys(radius.semantic) as (keyof typeof radius.semantic)[]).map((k) => (
              <TextInput
                ...
                mono
                label={`--radius-${k}`}
                value={radius.semantic[k]}
                onChange={(v) => setSemantic(k, v)}
              />
            ))}
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
            {(Object.keys(radius.primitives) as (keyof typeof radius.primitives)[]).map((k) => (
              <TextInput
                ...
                mono
                label={`--radius-${k}`}
                value={radius.primitives[k]}
                onChange={(v) => setPrimitive(k, v)}
              />
            ))}
          </div>
        </section>
