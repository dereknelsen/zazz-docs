...
1. `foundation/_layers.css`: declares `@layer variables, reset, legacy, zazz, migrations;` ([why this comes first](/docs/core-files/layers))
2. `foundation/_variables.css`: global design tokens (and the inline `@property` registrations)
3. `foundation/_reset.css`: element baselines
4. `foundation/_typography.css`: type system and `.prose`
5. `foundation/_view-transitions.css`: cross-document page transitions
6. `ui/_*.css` component files, in any order
7. `foundation/_utilities.css`, then `foundation/_layout.css`: atomic overrides and the container/layout grid (load last)
...
These map to `view-transition-name` values in `foundation/_view-transitions.css`. The `<main>` content animates on navigation; header and footer stay put.
