...
```html
<label class="w-checkbox checkbox-group">
  <input type="checkbox" class="w-checkbox-input w-checkbox-input--inputType-custom checkbox" />
  <span class="form-label">Checkbox label</span>
</label>
```

The `checkbox-group` wrapper sets up flex alignment with `--gap-xs` between the input and the label. The `w-checkbox-input` classes are Webflow's custom-styled-checkbox utilities that hide the native UI; the `checkbox` class applies Zazz styling on top.
...
The checked state uses Webflow's `w--redirected-checked` modifier class. In other frameworks, the equivalent rule is:
...
| Webflow | `checkbox` class on a custom-styled `w-checkbox-input`. Group wrapper is `w-checkbox checkbox-group`. |
| CSS / Tailwind | The same class after [installing utilities.css](/docs/quickstart/css). |

## Where to next

<Cards>
  <Card title="Radio" href="/docs/primitives/radio" description="The mutually-exclusive sibling. Same wrapper pattern." />
  <Card title="Switch" href="/docs/primitives/switch" description="A visual toggle for binary settings, built on the checkbox semantic." />
</Cards>
